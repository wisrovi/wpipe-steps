...
class HFTextGenerationStep(BaseStep):
    ...
    def execute(self, data: Dict[str, Any]) -> Dict[str, Any]:
        if pipe is None:
            ...
            pipe = pipeline(task="text-generation", model=self.model_name, device=self.device, local_files_only=True)
        prompt = data.get("prompt", "")
        if not prompt:
            data[self.response_key] = {"error": "No prompt"}
            return data
        result = pipe(prompt, max_length=self.max_length, temperature=self.temperature)
