# 🧱 SearchNearbyWithDistanceContext

Context for searching nearby members with distance in Redis geo.

## 📂 Structure
This step follows the WPipe professional package structure:
- `config/`: Configuration constants.
- `examples/`: Code examples.
- `exceptions/`: Custom step exceptions.
- `schemas/`: Pydantic input/output schemas.
- `states/`: Actual step execution logic.
- `utils/`: Internal helper utilities.
- `wrappers/`: Third-party library wrappers.

## ⚙️ Configuration
Namespace: `wpipe_steps.database.redis.geo.search_nearby_with_distance_sync`

## 🚀 How to Use
```python
from wpipe_steps.database.redis.geo.search_nearby_with_distance_sync import SearchNearbyWithDistanceContext
```
